...
- **The pipeline cache keeps four caches beside this run's**, oldest first to go, and matches its own
  filename grammar. It used to delete every other one — but a name carries the card, the driver and
  the shader tree, so the others are what a second card or the tree before the last edit compiled to.
  Deleting them made every switch a cold compile, which "every card from Turing up" turns from a
  curiosity into a normal case.
- **A second render from one placement waits for the frame that read the slot**, which is what makes
