...
**`SlotTable<Row>`** — `components/rtxvulkan/slottable.hpp`. Host rows, one device copy per frame
slot, one owed set per copy. `write(at)` marks the row owed by every copy; `resize(rows)` owes what
it appended and nothing else, because a row keeps its offset; `sync(slot, graveyard)` pays one copy
and clears it; `owes(slot)` is what an early return asks.

**One subscription for the whole system.** `updateInstanceRecords` now names in an out-parameter
every slot it wrote, and both backends are driven by that one list. `SceneAcceleration::place` and
...
**Migrated:** the acceleration structure's instance rows and the shading table's instance rows —
the two the terrain blink came out of. `mRows`, `mRowFlags`' partner buffers, `Tables::mInstances`,
`Tables::mRowsOwed` and `SceneAcceleration::mRowsOwed` are gone.

**Not yet migrated:** `mMaterialsOwed` with its `mMaterialCount`, and the two `BlockedBuffer` cases
— `mNormalsOwed` and `mPositionsOwed` — which are addressed by mesh range rather than by row and
want a `SlotBlocks` sibling. Three ledgers left of the five.

**Nine tests** in `apps/components_tests/rtx/slottable.cpp` ask the bookkeeping directly:
`HostBuffer` is write-combining and hands out no readable pointer, so what a copy holds cannot be
read back, but what it is *owed* can — and that is where every failure lived. One of them was
written because the first cut of `sync` doubled the buffer unconditionally and ran a streaming bench
out of device memory in twenty-two frames.
...
class rather than another instance. It costs nothing — 67.3 fps against 67.7, back to back.

## What is left

The residual 26 swings are not this mechanism, or not only this mechanism. §4's nineteen crossings
are inside that count and are the next thing to remove. After that the same measurement is the way to
ask whether the three remaining ledgers are carrying one too.
