...
namespace Rtx
{
    ...
    /// the order the dead left in.
    ///
    /// **`apps/rtxtool/repeatable.sh` is what asks it.** The gate puts the question to every scene
    /// column of every pair, so a take that went back to the last one freed fails there rather
    /// than going quiet until somebody measures again.
    ///
