...
namespace Rtx
{
    ...
    /// walk, and a thread asleep is counted by a sampling profiler as nothing at all: perf reads
    /// the trace call at microseconds a frame where the wall clock reads a fraction of a
    /// millisecond.
    ///
    ...
    ///
    /// **Why `Place` is split at all**: its worst frame is hundreds of times its median, and a
    /// profile cannot say which half — an arrival frame's time is in the driver, and the driver
    /// carries no frame pointer for perf to walk.
    ...
    ///
    /// **One table, and the count and the walk are derived from it.** Three lists kept level by
    /// hand is a row added to the enum reaching a report only if somebody writes it out twice
    /// more. `Rtx::NamedEnum` says at length why an enum states its spellings once.
    ...
    /// different place, and a signature that names them one by one is six parameters of one type
