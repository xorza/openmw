...
namespace RtxTool
{
    namespace
    {
        ...
        {
            EXPECT_EQ((BenchSpan{ .mFrames = 240 }).getFrames(MWRender::sStepSeconds), 240u);
            ...
            EXPECT_EQ((BenchSpan{ .mSeconds = 10.0f }).getFrames(MWRender::sStepSeconds), 600u);
            ...
            EXPECT_EQ((BenchSpan{ .mSeconds = 0.001f }).getFrames(MWRender::sStepSeconds), 1u);

            // Nothing asked for is nothing measured, which is what an absent warm-up says.
            EXPECT_TRUE(BenchSpan{}.empty());
            EXPECT_EQ(BenchSpan{}.getFrames(MWRender::sStepSeconds), 0u);
            ...
            EXPECT_EQ((BenchSpan{ .mFrames = BenchSpan::sUntilClosed }).getFrames(MWRender::sStepSeconds),
                BenchSpan::sUntilClosed);
            ...
            EXPECT_EQ(spec.getMeasured(MWRender::sStepSeconds), 600u);
            EXPECT_EQ(spec.getWarmup(MWRender::sStepSeconds), 120u);
        }
    }
}
