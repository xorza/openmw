...
namespace Rtx::Testing
{
    namespace
    {
        ...
        std::unique_ptr<Renderer> buildRenderer(std::string& reason)
        {
        ...
        /// exit — and this binary keeps up to three: a raw device for the tests that drive Vulkan
        /// directly, in a validated and an unvalidated flavour, and one `Renderer` for the pixel
        /// suite. Closing them here is both the fix and where they belonged: a cache that lives for
        /// the run should end with the run, not with the process.
    ...
    {
        return rendererCache().get(reason, buildRenderer);
    }
