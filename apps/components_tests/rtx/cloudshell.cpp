...
namespace
{
    ...
    float fadesTo(const Rtx::CloudShell& shell, float reach)
    {
        ...
        if (reach <= shell.mRings.y())
            return ramp(
                1.0f, Rtx::Shaders::CLOUD_RING_ALPHA, reach - shell.mRings.x(), shell.mRings.y() - shell.mRings.x());

        return ramp(
            Rtx::Shaders::CLOUD_RING_ALPHA, 0.0f, reach - shell.mRings.y(), shell.mRings.z() - shell.mRings.y());
    }

    /// Where the shader puts a direction, in sheet coordinates. `cloudDeck` in `sky.glsl`, without
    /// the storm's turn and the scroll.
    osg::Vec2f landsAt(const Rtx::CloudShell& shell, const osg::Vec3f& direction)
    {
    ...
    {
        const Rtx::CloudShell shell = Rtx::readCloudShell(*makeLayer());
        ...
        const Rtx::CloudShell mirrored
            = Rtx::readCloudShell(*makeLayer(sHeight, sCurvature, sTile, /*handedness=*/1.0f));
        ...
        const Rtx::CloudShell shifted
            = Rtx::readCloudShell(*makeLayer(sHeight, sCurvature, sTile, -1.0f, osg::Vec2f(2.5f, -1.5f)));
    ...
    TEST(RtxCloudShellTest, aRayLandsWhereTheMeshPinsTheSheet)
    {
        ...
        const Rtx::CloudShell shell = Rtx::readCloudShell(*layer);
    ...
    TEST(RtxCloudShellTest, theDeckEndsWhereTheEnginesOwnAlphaEndsIt)
    {
        ...
        EXPECT_NEAR(fadesTo(shell, 1.25f), 0.5f * (1.0f + Rtx::Shaders::CLOUD_RING_ALPHA), 1.0e-5f);
        EXPECT_NEAR(fadesTo(shell, 1.5f), Rtx::Shaders::CLOUD_RING_ALPHA, 1.0e-5f);
        EXPECT_NEAR(fadesTo(shell, 1.75f), 0.5f * Rtx::Shaders::CLOUD_RING_ALPHA, 1.0e-5f);
    ...
    {
        const Rtx::CloudShell shell
            = Rtx::readCloudShell(*makeLayer(sHeight, sCurvature, sTile, -1.0f, osg::Vec2f(), /*perRing=*/8));
    ...
    TEST(RtxCloudShellTest, theTransformOverAMeshIsPartOfWhereItHangs)
    {
        ...
        const Rtx::CloudShell dropped = Rtx::readCloudShell(*lowered);
        ...
        const Rtx::CloudShell doubled = Rtx::readCloudShell(*enlarged);
    ...
    {
        const Rtx::CloudShell flat = Rtx::readCloudShell(*makeLayer(sHeight, /*curvature=*/0.0f));
        ...
        EXPECT_NEAR(landsAt(Rtx::readCloudShell(*makeLayer()), slanted).x(), 0.70970f, 1.0e-4f);

        // A mesh that rises as it recedes is not a layer over anything, and is taken as the flat one.
        const Rtx::CloudShell domed = Rtx::readCloudShell(*makeLayer(sHeight, /*curvature=*/-0.0002f));
    ...
    {
        EXPECT_EQ(Rtx::readCloudShell(*new osg::Group).mTiles, osg::Vec2f());
        ...
        EXPECT_EQ(Rtx::readCloudShell(*bare).mTiles, osg::Vec2f());
        ...
        EXPECT_EQ(Rtx::readCloudShell(*flat).mTiles, osg::Vec2f());
    }
}
